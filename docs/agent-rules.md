...
**Re-apply** / **Overwrite the file** rewrites an edited block. See the state table below.
...
| Edited on disk              | Toolport wrote this block and it has been changed in the file since. Toolport leaves it alone on every automatic apply (startup, saving, switching sets, toggling clients) until you choose: **View diff** shows the file against the set, **Pull into set** makes the file's version your unsaved draft, **Overwrite the file** (or **Re-apply**) puts the set back. A change to the set itself is a newer revision and is written, so pull first if you want to keep the edit. |
